...
function loop(timestamp){
   ...
   if(boss_fight){
      ...
      if(timestamp-runtime>150){ if(!dog_freeze){rechts_dog++}; runtime = timestamp;
...
function tutorial() {
  ...
  const tut_text = document.createElement("div");
  const back_button = document.createElement("button");
  const skip_button = document.createElement("button");

  tut.style.width = document.body.offsetWidth*0.5 +"px";
  tut.style.height = document.body.offsetHeight*0.8 +"px";
  tut.style.backgroundImage = `url("images/tut${tut_nummer}.png")`
  tut.style.backgroundSize = "cover";
  tut.style.position = "fixed";    // bleibt sichtbar
   tut.style.top = "50%";
   tut.style.left = "50%";
   tut.style.transform = "translate(-50%, -50%)"; // zentrieren
   tut.style.border = "10px solid black";          // zur Kontrolle
   tut.style.zIndex = "1000";                     // über allem

   tut_text.style.width = tut.style.width;
  tut_text.style.height = parseInt(tut.style.height)/10 +"px"; // niedriger als das Bild
  tut_text.style.backgroundColor = "#000000ff";
  tut_text.style.border = "10px solid black";
  tut_text.style.borderTop = "none"; // damit es wie angeklebt aussieht
  tut_text.style.position = "fixed";
  tut_text.style.top = "calc(90%)"; // direkt unter tut (300/2)
  tut_text.style.left = "50%";
  tut_text.style.transform = "translateX(-50%)";
  tut_text.style.zIndex = "1000";
  tut_text.style.display = "flex";
  tut_text.style.justifyContent = "center";
  tut_text.style.alignItems = "center";
  tut_text.style.fontSize = "18px";
  tut_text.style.fontFamily = "Arial, sans-serif";

  tut_text.textContent = "Use this buttons to move";
  tut_text.style.color = "white";
  ...
  back_button.style.position = "absolute";
   ...
   skip_button.style.position = "absolute";
   skip_button.style.bottom = "10px";
   skip_button.style.right = "10px";

   [back_button, skip_button].forEach(btn => {
  btn.style.backgroundColor = "#38ee38ff";  // hellgrün
  btn.style.border = "2px solid black";
  btn.style.padding = "10px 20px";        // größer machen
  btn.style.margin = "5px";
  btn.style.fontSize = "16px";
  btn.style.borderRadius = "8px";         // abgerundet
  ...
  document.body.appendChild(tut_text);

  tut.appendChild(back_button);
  tut.appendChild(skip_button);

  function updateBackground() {
    tut.style.backgroundImage = `url("images/tut${tut_nummer}.png")`;
  }
  function updateText(){
   if(tut_nummer==1){tut_text.textContent = "Use this buttons to move";}
   if(tut_nummer==2){tut_text.textContent = "Escape the Pacman and pass by walls (some are fake and some invisible)";}
   if(tut_nummer==3){tut_text.textContent = "Heal here and be invisible";}
   if(tut_nummer==4){tut_text.textContent = "Shoot with these. The dog is coming";}
   if(tut_nummer==5){tut_text.textContent = "While moving you reload the ice super power, click on it to freeze the opponents"; skip_button.textContent = "finish";skip_button.style.backgroundColor = "orange";}
  }

  updateBackground();

  // WICHTIG: Promise hier zurückgeben
  return new Promise((resolve, reject) => {
    back_button.addEventListener("click", () => {
      tut_nummer -= 1;
      skip_button.textContent = "skip";skip_button.style.backgroundColor = "#38ee38ff";
      updateText()
      if (tut_nummer < 1) tut_nummer = 1;
      updateBackground();
    });
    ...
    skip_button.addEventListener("click", () => {
      ...
      if (tut_nummer > 5) {
        tut.remove();
        tut_text.remove();
        resolve("Tutorial beendet ✅"); // löst das Promise auf
      } else {
        updateBackground();
      }
    });

    // optional: reject nach Timeout
    // setTimeout(() => reject("Tutorial zu lange offen ⏱️"), 60000);
  });
}
